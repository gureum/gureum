...
enum ConfigurationName {
    ...
    static let emojiOverHanja = "EmojiOverHanja"
}
...
public class Configuration: UserDefaults {
    ...
    override init?(suiteName: String?) {
        ...
        register(defaults: [
            ...
            ConfigurationName.emojiOverHanja: false,
        ])
    }
    ...
    /// 이모지를 한자보다 우선적으로 보여주기.
    var emojiOverHanja: Bool {
        get {
            return bool(forKey: ConfigurationName.emojiOverHanja)
        }
        set {
            `set`(newValue, forKey: ConfigurationName.emojiOverHanja)
        }
    }
}
