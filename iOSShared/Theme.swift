...
public class Theme {
    public func dataForFilename(name _: String) -> Data? {
        assert(false)
        return nil
    }
    ...
    public func imageForFilename(name: String) -> UIImage? {
        return imageForFilename(name: name, withTopMargin: 0)
    }
    ...
    public func traitForName(traitName: String, topMargin: CGFloat) -> ThemeTrait {
        if let traits = self.mainConfiguration["trait"] as? NSDictionary {
            if let traitFilename = traits[traitName] as? String {
                var error: NSError?
                let traitData: Any! = jsonObjectForFilename(name: traitFilename, error: &error)
    ...
    public func traitForSize(size: CGSize) -> ThemeTrait {
        ...
        case 320.0, 375.0, 414.0:
            return phonePortraitConfiguration
        case 480.0:
            return phoneLandscape480Configuration
        case 568.0, 667.0:
            return phoneLandscape568Configuration
        case 768.0:
            return padPortraitConfiguration
        case 1024.0:
            return padLandscapeConfiguration
        default:
...
public class ThemeCaptionGroup {
    ...
    func classByName(key: String) -> ThemeCaptionClass {
        return ThemeCaptionClass(trait: trait, configuration: trait.configuration[self.group + "-" + key])
    }
    ...
    public func row(row: Int) -> ThemeCaptionClass {
        return classByName(key: "row-\(row)")
    }

    public func key(key: String) -> ThemeCaptionClass {
        return classByName(key: "key-\(key)")
    }

    public func caption(key: String) -> ThemeCaptionClass {
        return classByName(key: "caption-\(key)")
    }
}
...
public class ThemeCaption {
    ...
    lazy var topMargin: CGFloat = { self.needsMargin ? self.trait.topMargin : 0 }()
    ...
    func appealButton(button: GRInputButton) {
        let (image1, image2, image3) = backgroundImages
        // assert(image1 != nil)
        ...
        button.effectBackgroundImage = effectBackgroundImage
    }
    ...
    func arrangeButton(button: GRInputButton) {
        ...
        arrangeEffectView(button: button)
    }
    ...
    func arrangeEffectView(button: GRInputButton) {
        ...
        button.effectView.backgroundImageView.image = effectBackgroundImage
        let insets = effectEdgeInsets
...
//        frame.origin.y = button.frame.origin.y - insets.top
        ...
        let position = effectPosition
        frame.origin.x += position.x
        frame.origin.y += position.y + topMargin / 2
    ...
    func _backgroundImages() -> (UIImage?, UIImage?, UIImage?) {
        if let imageNames = self.attributeByGetter(getter: { $0._backgroundImageNames() }, until: { ($0! as AnyObject).count > 0 }) as? [String] {
            ...
            for imageName in imageNames {
                let image = trait.owner.imageForFilename(name: imageName, withTopMargin: topMargin)
...
class BuiltInTheme: Theme {
    override func dataForFilename(name _: String) -> Data? {
        return nil
        /*
...
class ThemeResourceCoder {
    ...
    func encodeFromData(data: NSData) -> String {
        let encoded = data.encryptedAES256Data(withKey: key() as Data)!
    ...
    func decodeToData(data: String) -> NSData {
        ...
        return encoded!.decryptedAES256Data(withKey: key() as Data?)! as NSData
    }
