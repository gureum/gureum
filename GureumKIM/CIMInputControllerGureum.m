...
- (IBAction)checkRecentVersion:(id)sender {
    ...
    if ([recentVersion isEqualToString:currentVersion]) {
        NSString *msg = [NSString stringWithFormat:@"현재 사용하고 있는 구름 입력기 %@ 는 최신 버전입니다.", currentVersion];
        NSAlert *alert = [NSAlert alertWithMessageText:@"구름 입력기 업데이트 확인" defaultButton:@"확인" alternateButton:nil otherButton:nil informativeTextWithFormat:msg];
        [alert beginSheetModalForWindow:nil modalDelegate:nil didEndSelector:nil contextInfo:NULL];
    } else {
        NSString *msg = [NSString stringWithFormat:@"현재 사용하고 있는 구름 입력기는 %@ 이고 최신 버전은 %@ 입니다.", currentVersion, recentVersion];
        if (recentDownload.length == 0) {
            msg = [msg stringByAppendingString:@" 곧 업데이트 링크가 준비될 예정입니다."];
        }
        NSAlert *alert = [NSAlert alertWithMessageText:@"구름 입력기 업데이트 확인" defaultButton:@"확인" alternateButton:nil otherButton:nil informativeTextWithFormat:msg];
        [alert beginSheetModalForWindow:nil modalDelegate:nil didEndSelector:nil contextInfo:NULL];
        if (recentDownload.length > 0) {
